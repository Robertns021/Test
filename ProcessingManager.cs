...
namespace ProcessingModule
{
    ...
    public class ProcessingManager : IProcessingManager
    {
        ...
        PointIdentifier digitalOut3 = new PointIdentifier(PointType.DIGITAL_OUTPUT, 2002);
        PointIdentifier digitalOut4 = new PointIdentifier(PointType.DIGITAL_OUTPUT, 1000);
        List<PointIdentifier> pointList = new List<PointIdentifier> { digitalOut1, digitalOut2, digitalOut3, digitalOut4 };
        List<IPoint> points = storage.GetPoints(pointList);

            if ((points[0].RawValue == 1 || points[1].RawValue == 1 || points[2].RawValue == 1) && (pointAddress == 1000 && value == 1))
            {
                return;
            }

            if (points[3].RawValue == 1 && (pointAddress == 2000 || pointAddress == 2001 || pointAddress == 2002))
            {
                return;
            }
